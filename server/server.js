...
// ✅ Webhook BEFORE JSON middleware (for Clerk)
app.post("/clerk", express.raw({ type: "application/json" }), clerkWebhooks);
...
// ✅ Admin routes with full protection
app.use(
  '/api/admin',
  adminRateLimit,
  adminSecurityHeaders,
...
// ✅ Conditional Clerk Middleware (to avoid dev errors)
const contentAuth = process.env.CLERK_SECRET_KEY
  ? clerkMiddleware()
  : (req, res, next) => next();
app.use('/api/content', contentAuth, contentRouter);

// ✅ Connect to MongoDB (Vercel handles connection reuse automatically)
connectDB()
